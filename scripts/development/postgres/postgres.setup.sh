...
envsubst < $PATRONI_CONFIG_YAML | kubectl apply -n "$NAMESPACE" -f -
envsubst < $PATRONI_STATEFULSET_YAML | kubectl apply -n "$NAMESPACE" -f -
envsubst < $PATRONI_POD_DISCOVERY_YAML | kubectl apply -n "$NAMESPACE" -f -
